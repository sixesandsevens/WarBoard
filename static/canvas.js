  ...
  const allCtxMenus = [mapCtx, mapCtxBg, mapCtxLayers, mapCtxClear, penCtx, shapeCtx, textCtx, rulerCtx, assetCtx];
  ...
  function clearRoomBackground() {
    send("ROOM_SETTINGS", { background_mode: "solid", background_url: null });
    log("Background cleared.");
  }

  function fitRectToView(width, height, fill = 0.88) {
    const w = Math.max(1, Number(width || 0));
    const h = Math.max(1, Number(height || 0));
    if (w < 1 || h < 1) return false;
    const vw = Math.max(1, canvas.clientWidth || canvas.width || 1);
    const vh = Math.max(1, canvas.clientHeight || canvas.height || 1);
    const z = clamp(Math.min(vw / w, vh / h) * clamp(fill, 0.2, 0.98), 0.08, 8);
    cam.z = z;
    cam.x = (vw - (w * z)) / 2;
    cam.y = (vh - (h * z)) / 2;
    requestRender();
  ...
  function renderAssetGrid() {
    ...
    assetGridEl.querySelectorAll("button[data-asset-action]").forEach((btn) => {
      ...
      btn.onclick = (e) => {
        ...
        if (action === "kind-map") {
          setAssetKindOverride(a, "map");
          closeAssetKindMenus();
          return;
        }
        if (action === "kind-piece") {
          setAssetKindOverride(a, "piece");
          closeAssetKindMenus();
          return;
        }
        if (action === "kind-unknown") {
          setAssetKindOverride(a, "unknown");
          closeAssetKindMenus();
          return;
        }
        if (action === "kind-auto") {
  ...
  const applyToolColor = () => {
    ...
    if (colorPanelTargetShapeId) {
      const sh = state.shapes.get(colorPanelTargetShapeId);
      if (sh && canEditShapeLocal(sh)) {
        send("SHAPE_UPDATE", { id: colorPanelTargetShapeId, color: String(toolColorPicker.value || "#ffffff"), commit: true });
      }
    }
    refreshToolContextChecks();
  };
  if (toolColorPicker) toolColorPicker.addEventListener("input", applyToolColor);
  if (toolColorPicker) toolColorPicker.addEventListener("change", applyToolColor);
  if (toolSizeSlider) toolSizeSlider.addEventListener("input", () => {
  ...
  if (mapCtx) mapCtx.addEventListener("mousemove", (e) => {
  ...
  if (mapCtx) mapCtx.addEventListener("mouseleave", () => scheduleCtxSubHide(120));
  [mapCtxBg, mapCtxLayers, mapCtxClear].filter(Boolean).forEach((sub) => {
  ...
  window.addEventListener("mousedown", (e) => {
    ...
    if (!e.target.closest("[data-asset-card='1']")) closeAssetKindMenus();
    hideTooltip();
  });
  ...
  if (assetSetApplyBtnEl) assetSetApplyBtnEl.addEventListener("click", () => {
    ...
    if (!id) {
      toast("Select a saved set first.");
      return;
    }
    const row = (assetState.savedSets || []).find((item) => item.id === id);
    if (!row) {
      toast("Saved set was not found.");
      renderAssetSavedSets();
      return;
    }
  ...
  if (assetUploadBtnEl) assetUploadBtnEl.onclick = async () => {
    ...
    if (!file) {
      log("ASSET UPLOAD ERROR: choose a file first");
      return;
    }
    try {
  ...
  if (assetZipUploadBtnEl) assetZipUploadBtnEl.onclick = async () => {
    ...
    if (!file) {
      log("ASSET ZIP ERROR: choose a zip file first");
      return;
    }
    try {
  ...
  document.getElementById("tokenMenuGroup").onclick = () => {
    groupSelectedTokens();
    closeTokenMenu();
  };
  document.getElementById("tokenMenuUngroup").onclick = () => {
  ...
  document.getElementById("tokenMenuDelete").onclick = () => {
    if (!tokenMenuTokenId) return;
    const t = state.tokens.get(tokenMenuTokenId);
    if (!canDeleteTokenLocal(t)) return;
  ...
  uploadBgEl.addEventListener("click", async () => {
    if (!isGM()) {
      log("Upload BG ERROR: GM only");
      return;
    }
    const rid = roomEl.value.trim();
    if (!rid) {
      log("Upload BG ERROR: room id required");
      return;
    }
    const file = bgFileEl.files && bgFileEl.files[0];
    if (!file) {
      log("Upload BG ERROR: choose an image file first");
      return;
    }
    try {
